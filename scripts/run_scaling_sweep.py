...
app = modal.App("blackwell-scaling-sweep")
...
# [Include V6 Kernel Code Here - Same as before]
CUDA_SOURCE = r'''
...
def run_benchmark():
    ...
    print(f"Sweep Benchmark on {num_total_gpus} GPUs", flush=True)
    ...
    # SWEEP CONFIG
    batches = [256, 384, 512, 768]
    gpu_counts = range(1, num_total_gpus + 1)

    print(f"\n{'BATCH':<8} | {'GPUS':<6} | {'LATENCY (ms)':<15} | {'THROUGHPUT (tok/s)':<20}")
    print("-" * 60)

    for b in batches:
        # Pre-allocate inputs on GPU 0
        topk = 2048
        ...
        for n_gpus in gpu_counts:
            ...
            def _worker(gpu_id, qn_i, qp_i, idx_i):
                with torch.cuda.device(gpu_id):
                    # Simulate Non-Blocking Transfer from Source
            ...
            start = time.time()
            for _ in range(10): # 10 iters for speed
                futures = []
            ...
            lat = ((end - start) / 10) * 1000
            tput = b / (lat / 1000)

            print(f"{b:<8} | {n_gpus:<6} | {lat:<15.4f} | {tput:<20.2f}")
...
def run_sweep():
    ...
    import subprocess
    print("Launching Scaling Sweep...", flush=True)
