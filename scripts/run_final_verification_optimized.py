...
app = modal.App("blackwell-final-verification-opt")
...
def compile_kernel():
    return load_inline(
        name="dsa_attn_benchmark_opt_v2",
...
def run_benchmark():
    ...
    print(f"Running OPTIMIZED VERIFICATION on {num_total_gpus} GPUs", flush=True)

    # 1. Warmup & Pre-Allocate Buffers (Cache + Output)
    print("Pre-allocating buffers on all GPUs...", flush=True)
    ...
    def setup_gpu(gpu_id):
        with torch.cuda.device(gpu_id):
            # Pre-allocate read-only Cache (Simulates server state)
    ...
    for i, bufs in enumerate(results):
        gpu_buffers[i] = bufs
    ...
    for b in configs:
        topk = 2048
        # Inputs on GPU0 (Simulate incoming request)
        ...
        def _worker(gpu_id, qn_i, qp_i, idx_i):
            with torch.cuda.device(gpu_id):
                ...
                # Transfer Inputs (Simulate scatter)
        ...
        for _ in range(20):
            futures = []
            if n_gpus == 1:
                # Direct Path Logic for Batch 128
                _worker(0, qn_splits[0], qp_splits[0], idx_splits[0])
            else:
...
def run_verify_opt():
    ...
    import subprocess
    print("Launching OPTIMIZED Verification...", flush=True)
    subprocess.run(["python", "run_benchmark.py"], check=True)

@app.local_entrypoint()
def main():
    run_verify_opt.remote()
